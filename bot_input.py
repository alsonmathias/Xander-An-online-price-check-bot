...
async def first_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Please send the first product URL.")
    return PRODUCT1
...
async def second_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Thank you! Now please send the second product URL.")
    return PRODUCT2

# Handler to store the URL of second product
async def budget_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['budget_price'] = update.message.text
...
async def check_prices(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    budget_price = context.user_data.get('budget_price')
    budget = context.user_data.get('budget')

    #Check if URLs and budget are set
    if not second_url or not budget_price or not budget:
        await update.message.reply_text("Please set the products first using /addproduct.")
        return

    # Retrieve prices for both the products
    first_price = find_price(second_url)
    second_price = find_price(budget_price)
