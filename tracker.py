    ...
    try:
        ...
        # Check if the URL is of Flipkart page
            ...
            if price_tag:
                ...
                price = round(float(price_text), 2)
                return price
        #Check if the URL is of Snapdeal Page
        elif 'snapdeal' in URL:
            price_tag = soup.find(class_="payBlkBig")
            if price_tag:
                price_text = price_tag.get_text().replace("₹", "").replace(",", "")
                price = round(float(price_text), 2)
                return price

        # Check if the URL is of Myntra page
        elif 'myntra' in URL:
            price_tag = soup.find(class_="pdp-price")
            if price_tag:
                price_text = price_tag.get_text().replace("₹", "").replace(",", "")
                price = round(float(price_text), 2)
                return price
    #To provide exception using except for every try
